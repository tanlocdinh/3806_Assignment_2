...
# =============================================================================
_QC_HIT = re.compile(
    r"(?i)(counterexample\s+found|Quickcheck\s+found\s+a\s+counterexample)"
)
_NP_HIT = re.compile(
    r"(?i)(Nitpick\s+found\s+a\s+counterexample|genuine\s+counterexample)"
)


def precheck_quickcheck_refutes(
    isabelle, session_id: str, steps_with_candidate: List[str], timeout_s: int
) -> bool:
    cmd = f"quickcheck[timeout = {int(timeout_s)}]"
    thy = build_theory(
        steps_with_candidate + [cmd], add_print_state=False, end_with="sorry"
    )
    for msg in _iter_note_messages(
        run_theory(isabelle, session_id, thy, timeout_s=max(1, int(timeout_s) + 2))
    ):
...
_SLEDGE_BY = re.compile(
    r"(?i)(?:try this:\s*)?(by\s+\([^)]+\)|by\s+\w+(?:\s+[^)\n]*)?)", re.MULTILINE
)
_SLEDGE_APPLY = re.compile(
    r"(?i)(?:try this:\s*)?(apply\s+\([^)]+\)|apply\s+\w+(?:\s+[^)\n]*)?)", re.MULTILINE
)
_SLEDGE_METIS_LINE = re.compile(r"(?i)^.*?metis\b.*?:\s*(.*)$", re.MULTILINE)
_SLEDGE_SIMP_LINE = re.compile(
    r"(?i)(?:try this:\s*)?(simp(?:\s+add:\s*[^)\n]*)?)", re.MULTILINE
)
_SLEDGE_AUTO_LINE = re.compile(
    r"(?i)(?:try this:\s*)?(auto(?:\s+[^)\n]*)?)", re.MULTILINE
)
...
def _extract_sledge_by_lines(text: str) -> List[str]:
    ...
    for match in _SLEDGE_METIS_LINE.finditer(text):
        facts = (
            match.group(1).strip() if match.lastindex and match.lastindex >= 1 else ""
        )
        suggestion = _normalize_finisher_text(
            f"by (metis {facts})" if facts else "by (metis)"
        )
        if suggestion and suggestion not in out:
            out.append(suggestion)
    ...
    for line in text.split("\n"):
        ...
        if any(
            word in line_lower
            for word in ["simp", "auto", "blast", "force", "fastforce"]
        ):
...
def sledgehammer_finishers(
    isabelle, session_id: str, steps: List[str], timeout_s: int = 5, limit: int = 5
) -> List[str]:
    ...
    all_text = ""
    responses = run_theory(
        isabelle, session_id, thy, timeout_s=max(1, int(timeout_s) + 2)
    )
...
def _build_find_theorems_theory(symbols: List[str]) -> str:
    body = "\n".join(f"find_theorems name:{s}" for s in symbols[:8])
    imports = os.environ.get("ISABELLE_IMPORTS", "Main").strip() or "Main"
...
#         for flt in filters:
#             lines.append(
#                 f"find_theorems {flt} name:{s}" if flt else f"find_theorems name:{s}"
#             )
#     return textwrap.dedent(
#         "theory FT2_Scratch\nimports Main\nbegin\n\n{}\n\nend".format("\n".join(lines))
#     ).strip()
...
def mine_lemmas_from_state(
    isabelle, session_id: str, state_hint: str, max_lemmas: int = 6
) -> List[str]:
    ...
    lemmas, seen = [], set()
    for msg in _iter_note_messages(
        run_theory(isabelle, session_id, theory_text, timeout_s=10)
    ):
...
def mine_facts_prioritized(
    isabelle, session_id: str, state_hint: str, limit: int = 6
) -> List[str]:
    toks = list(dict.fromkeys(FIND_NAME_TOKENS.findall(state_hint)))
    if not toks:
        return []
    theory_text = _build_find_theorems_filtered(
        toks, ["simp", "intro", "rule", "elim", "dest"]
    )
    # Prefer *_def by name (cheap extra pass)
    def_name_patterns = [f'name: "{stem}_def"' for stem in toks[:6]]
    if def_name_patterns:
        theory_text += "\n" + "\n".join(
            f"find_theorems {p} - 10" for p in def_name_patterns
        )
    freq: Dict[str, int] = {}
    for msg in _iter_note_messages(
        run_theory(isabelle, session_id, theory_text, timeout_s=12)
    ):
        for line in msg.splitlines():
            m = _LEMMA_LINE.match(line.strip())
            if not m:
    ...
    return [
        k for k, _ in sorted(freq.items(), key=lambda kv: (-kv[1], len(kv[0]), kv[0]))
    ][:limit]
...
# =============================================================================
_IND_STEP = re.compile(
    r"^\s*apply\s*\(\s*induct(?:ion)?\s+([^)]+)\)\s*$", re.IGNORECASE
)
...
def _build_structured_induction_steps(
    goal: str, var_raw: str, nil_fin: str, cons_fin: str
) -> List[str]:
    return [
        f'lemma "{goal}"',
...
def _build_structured_cases_bool_steps(
    goal: str, var_raw: str, t_fin: str, f_fin: str
) -> List[str]:
    return [
        f'lemma "{goal}"',
...
def try_structured_variants(
    ...
    for s in final_steps:
        if ind_var is None:
            m = _IND_STEP.match(s)
            ind_var = m.group(1) if m else ind_var
        if cases_var is None:
            c = _CASES_STEP.match(s)
            cases_var = c.group(1) if c else cases_var
    ...
    if ind_var:
        ...
        for nil_fin in finishers:
            for cons_fin in finishers:
                if tries >= max_tries or left() <= 0:
                    return None
                steps = _build_structured_induction_steps(
                    goal, ind_var, nil_fin, cons_fin
                )
                ok, _ = finished_ok(
                    run_theory(
                        isabelle,
    ...
    if cases_var and _booleanish_goal(goal):
        ...
        for t_fin in finishers:
            for f_fin in finishers:
                if tries >= max_tries or left() <= 0:
                    return None
                steps = _build_structured_cases_bool_steps(
                    goal, cases_var, t_fin, f_fin
                )
                ok, _ = finished_ok(
                    run_theory(
                        isabelle,
...
def suggest_continuations(
    step: str, macro_map: Optional[Dict[str, List[Tuple[str, int]]]], k: int = 1
) -> List[str]:
    """
