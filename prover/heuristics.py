...
def suggest_common_lemmas(state_hint: str) -> List[str]:
    ...
    lemmas: List[str] = []
    if "real" in txt or "::real" in txt:
        lemmas += ["sq_nonneg", "abs_nonneg", "mult_self_nonneg"]
    if any(op in txt for op in ("≤", "<", "≥", ">")):
        lemmas.append("sq_nonneg")
    if "power" in txt or "^" in txt:
        lemmas += ["sq_nonneg", "power2_nonneg"]
    if "rev" in txt:
        lemmas.append("rev_rev_ident")
    if "append" in txt or " @ " in txt:
        lemmas.append("append_assoc")
    if "length" in txt and ("append" in txt or " @ " in txt):
        lemmas.append("length_append")
    if "map" in txt and ("append" in txt or " @ " in txt):
        lemmas.append("map_append")
    out, seen = [], set()
    for l in lemmas:
        if l not in seen:
            seen.add(l)
...
def _heuristic_score(
    cmd: str, goal: str, state: str, facts: Optional[List[str]] = None
) -> float:
    g = (goal + " " + state).lower()
    score = 0.0
    ...
    _is_real = "real" in g or "::real" in g or "float" in g
    _is_arith = _any_in(
        g, ("suc", "nat", "≤", "<", "0", "add", "mult", "+", "-", "*", "dvd")
    )

    if _is_arith and not _is_real:
        # nat/int arithmetic: linarith/arith are good; induction often needed
        if "induction" in cmd:
            score -= 0.8
        if "linarith" in cmd:
            score -= 0.7
    ...
    if _is_real:
        # real nonlinear arithmetic: strongly prefer nlinarith/argo/linarith
        if "nlinarith" in cmd:
            score -= 1.5
        if "argo" in cmd:
            score -= 1.2
        ...
        if "linarith" in cmd:
            score -= 0.8
        if "induction" in cmd:
            score += 0.8  # penalize induction for reals
...
def augment_with_facts_for_finishers(
    base_finishers: List[str], facts: List[str], cap: int = 8
) -> List[str]:
    if not facts:
        return base_finishers
    defs = [f for f in facts if f.endswith("_def")]
